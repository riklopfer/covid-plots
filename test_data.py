...
class CovidTrackingTest(unittest.TestCase):
    ...
    def test_zero_window(self):
        df = self.data_provider.load_state_test_df('pa', window=0)
    ...
    def test_window(self):
        df = self.data_provider.load_state_test_df('pa', window=3)
        test_window_columns = get_test_column_names(df)
        self.assertEqual(1, len(test_window_columns))
    ...
    def test_bad_abbrev(self):
        with self.assertRaises(ValueError):
            self.data_provider.load_state_test_df('usa', window=0)

    def test_usa_data(self):
        df = self.data_provider.load_usa_test_df(window=7)
